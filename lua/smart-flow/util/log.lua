...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.SmartFlow.config ~= nil and not _G.SmartFlow.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[smart-flow.nvim@%s] %s", scope, string.format(str, ...)),
        level,
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h SmartFlow.options` to read more."
        )
    end
end
